...
electron_log.catchErrors({ showDialog: true });
Object.assign(console, electron_log.functions);
require('dotenv').config({ path: path.join(__dirname, '..', '.env') });

try { require('electron-json-config'); } catch {
...
const tagsIP = process.env.TAGS_IP,
    musicIP = process.env.MUSIC_IP;
var players = [],
    numplayers = 0,
    key = config.get('key', '1'),
    ...
    inlobby = true,
    zoom = 1,
    gamemode = config.get('settings.gamemode', 0),
    gmode = config.get('settings.bwgmode', ''),
    guildlist = false,
    tagslist = [],
    guildtag = config.get('settings.gtag', true),
    startapi = null,
    starttime = new Date(),
    music = { session: false, playing: false, looping: false, queue: [], updatetimer: 0, timeratio: [0, 0], songtimer: 0, locked: false, lockwarned: false };
...
function updateHTML() {
    let namehtml = '',
        taghtml = '';
    wshtml = '', fkdrhtml = '', wlrhtml = '', bblrhtml = '', finalshtml = '', winshtml = '';
    //con.log('changing 1');
    if (!goodfile) {
        namehtml += '<li style="color: #FF0000">NO CLIENT PATH FOUND</li>';
        taghtml += '<li style="color: #FF0000">ERROR</li>';
        wshtml += '<li style="color: #FF0000">X</li>';
        fkdrhtml += '<li style="color: #FF0000">X</li>';
        wlrhtml += '<li style="color: #FF0000">X</li>';
        bblrhtml += '<li style="color: #FF0000">X</li>';
        finalshtml += '<li style="color: #FF0000">X</li>';
        winshtml += '<li style="color: #FF0000">X</li>';
        namehtml += `<li style="color: #FF0000">MISSING CLIENT LOGS FILE</li>`;
        taghtml += '<li style="color: #FF0000">ERROR</li>';
        wshtml += '<li style="color: #FF0000">X</li>';
        fkdrhtml += '<li style="color: #FF0000">X</li>';
        wlrhtml += '<li style="color: #FF0000">X</li>';
        ...
        namehtml += '<li style="color: #FF0000">MISSING/INVALID API KEY</li>';
        taghtml += '<li style="color: #FF0000">ERROR</li>';
        wshtml += '<li style="color: #FF0000">X</li>';
        fkdrhtml += '<li style="color: #FF0000">X</li>';
        wlrhtml += '<li style="color: #FF0000">X</li>';
        ...
        finalshtml += '<li style="color: #FF0000">X</li>';
        winshtml += '<li style="color: #FF0000">X</li>';
        namehtml += `<li style="color: #FF0000">RUN COMMAND "/api new"</li>`;
        taghtml += '<li style="color: #FF0000">ERROR</li>';
        wshtml += '<li style="color: #FF0000">X</li>';
        fkdrhtml += '<li style="color: #FF0000">X</li>';
        wlrhtml += '<li style="color: #FF0000">X</li>';
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        let stars = '',
            starsColor = '#AAAAAA';
        avatar = 'https://crafatar.com/avatars/ec561538f3fd461daff5086b22154bce?size=48&default=MHF_Steve&overlay'; //tagColor = '#AAAAAA';
        if (players[i].api === undefined) {
            ...
            namehtml += `<li style="background: url(${avatar}) no-repeat left center; background-size: 20px; padding-left: 25px;">${players[i].name}</li>`;
            taghtml += '<li style="color: #F3FF00">NICK</li>';
            wshtml += '<li>-</li>';
            fkdrhtml += '<li>-</li>';
            wlrhtml += '<li>-</li>';
            bblrhtml += '<li>-</li>';
            ...
            if (players[i].avatar !== undefined) {
                namehtml += players[i].namehtml;
                taghtml += players[i].taghtml;
                wshtml += players[i].wshtml;
                fkdrhtml += players[i].fkdrhtml;
                wlrhtml += players[i].wlrhtml;
                bblrhtml += players[i].bblrhtml;
                finalshtml += players[i].finalshtml;
                winshtml += players[i].winshtml;
            } else {
                stars = '[0✫] ';
                ttaghtml = '<li>-</li>';
                twshtml = '<li>0</li>';
                ...
                tfinalshtml = '<li>0</li>';
                twinshtml = '<li>0</li>';
                if (players[i].api.stats === undefined) {} else if (players[i].api.stats.Bedwars === undefined) {} else {
                    let tfkdr = 0,
                        twlr = 0,
                        tbblr = 0;
                    if (players[i].api.achievements !== undefined && players[i].api.achievements.bedwars_level !== undefined) {
                        stars = `[${players[i].api.achievements.bedwars_level}✫] `;
                        starsColor = starColor(players[i].api.achievements.bedwars_level);
                    }
                    ...
                    if (players[i].api.stats.Bedwars.final_kills_bedwars !== undefined && players[i].api.stats.Bedwars.final_deaths_bedwars !== undefined) tfkdr = parseFloat(players[i].api.stats.Bedwars.final_kills_bedwars / players[i].api.stats.Bedwars.final_deaths_bedwars).toFixed(2);
                    tfkdrhtml = `<li style="color: ${fkdrColor(tfkdr)}">${tfkdr}</li>`;
                    ...
                    if (players[i].api.stats.Bedwars.wins_bedwars !== undefined && players[i].api.stats.Bedwars.losses_bedwars !== undefined) twlr = parseFloat(players[i].api.stats.Bedwars.wins_bedwars / players[i].api.stats.Bedwars.losses_bedwars).toFixed(2);
                    twlrhtml = `<li style="color: ${wlrColor(twlr)}">${twlr}</li>`;
                    if (players[i].api.stats.Bedwars.beds_broken_bedwars !== undefined && players[i].api.stats.Bedwars.beds_lost_bedwars !== undefined) tbblr = parseFloat(players[i].api.stats.Bedwars.beds_broken_bedwars / players[i].api.stats.Bedwars.beds_lost_bedwars).toFixed(2);
                    tbblrhtml = `<li style="color: ${bblrColor(tbblr)}">${tbblr}</li>`;
                ...
                let tag = getTag(players[i].api, tagslist);
                ttaghtml = `<li style="color: ${tag[1]}">${tag[0]}</li>`;
                namehtml += `<li style="background: url(${avatar}) no-repeat left center; background-size: 20px; padding-left: 25px;">${stars}${players[i].namehtml}</li>`;
                taghtml += ttaghtml;
                wshtml += twshtml;
                fkdrhtml += tfkdrhtml;
                wlrhtml += twlrhtml;
                bblrhtml += tbblrhtml;
                finalshtml += tfinalshtml;
                winshtml += twinshtml;
            }
        }
    }
                ...
                success: (data) => {
                        if (data.success === true && data.player !== null) {
                            ...
                            if (data.player.displayname === ign) {
                                ...
                                got = true;
                                if (gamemode === 0) {
                                    let stars = '<span>[0✫]</span>',
                                        ttaghtml = '<li>-</li>',
                                        twshtml = '<li style="color: red">?</li>',
                                        tfkdrhtml = '<li>0</li>',
                                        twlrhtml = '<li>0</li>',
                                        tfinalshtml = '<li>0</li>',
                                        twinshtml = '<li>0</li>';
                                    let avatar = `https://crafatar.com/avatars/${api.id}?size=48&default=MHF_Steve&overlay`;
                                    if (api.stats === undefined) {} else if (api.stats.Bedwars === undefined) {} else {
                                        let tfkdr = 0,
                                            twlr = 0; //tbblr = 0;
